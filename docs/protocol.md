...
Result: `{windows:[{windowId, title, width, height, devicePixelRatio, window:<window node>, root:<tree>}], nodeCount, truncated?, omittedNodeCount?, nextHints?}`. Node fields include `nodeId, windowId, kind, renderKind?, type, typeAliases, styleItem?, objectName, qmlId?, implementationId?, visualPath, text?, enabled?, visible, opacity, bbox, insideViewport, viewport, selectors, sourceLocation?, sourceInstance?, frameworkInternal?, delegate?, properties?, children` (`actionable`/`interactable` computed only when requested via `fields`).

When the QmlAgent plugin is built with `Qt6::Quick3D`, QtQuick3D frontend objects below `View3D` are included automatically. The `View3D` boundary remains `kind:"QQuickItem"` and reports `renderKind:"QtQuick3D"`; its scene descendants report `kind:"QQuick3DObject"` without a render kind. QtQuick3D descendants are structural/source evidence only: they do not have a 2D `bbox` and must not be treated as `QQuickItem` input targets.

Objects inheriting `QCanvasPainterItem` are tagged with `renderKind:"QtCanvasPainter"`, including application-specific subclass names. When built with `Qt6::CanvasPainter`, requested Q_PROPERTY values containing public CanvasPainter brushes, gradients, patterns, shadows, images, paths, custom brushes, and offscreen canvases are serialized as structured application-exposed state. Path endpoint evidence is bounded and reports truncation and public-API limitations. None of this is independently observed paint execution: the current public API does not expose the retained draw-call sequence, shader/custom-brush payloads, or image source filenames.
...
| `Input.dispatchKeyEvent` | `key` string (QKeySequence portable name; aliases Escape/Delete/Insert/PageUp/PageDown/Enter/Spacebar accepted) or `keyCode` int; `type` = `keyClick` (default)\|`keyPress`\|`keyRelease`; `text`; `modifiers`; optional `nodeId`/`selector` (focuses target first); optional `windowId` (routes untargeted input to that 1-based `UI.getTree` window and validates targeted input belongs to it) | `keyCode, text, type, focus?, deliveryWindow`; diag `input.not_deliverable`; reasons `invalid_key`, `invalid_type`, `invalid_window_id`, `window_not_found`, `window_mismatch`, `focus_failed`, `unknown_window` |
| `Input.typeText` | `text` (required unless `replaceExisting`), optional `nodeId`/`selector` (focus first), optional `windowId` with the same routing/mismatch contract as key dispatch, `replaceExisting` bool = false (selectAll-then-type; empty text = clear), `focusSettle {timeoutMs}`, `settle` | `unitsSent, targetKind (window\|editableItem\|item), focus?, replaceExisting?, cursorSetup?, deliveryWindow`; diags `input.not_editable`, `input.window_fallback` (keys went to the window — proves nothing about a field), `input.replace_selection_failed`, `input.not_deliverable`; reasons `invalid_text`, `invalid_window_id`, `window_not_found`, `window_mismatch`, `focus_failed`, `not_editable`, `replace_selection_failed` |
